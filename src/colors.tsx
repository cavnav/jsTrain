...
function mix() {
  ...
  if (typeof lastObject == "object" && lastObject.constructor != Array) {
    ...
    options.hex =
      typeof customOptions.hex != "undefined" ? customOptions.hex : options.hex;
    arguments.length--;
  }
...
function findNearest(color, list) {
  ...
  listCopy.sort(function(c1, c2) {
    ...
    var euclideanDistance1 = Math.sqrt(
      Math.pow(c[0] - rgb1[0], 2) +
        Math.pow(c[1] - rgb1[1], 2) +
        Math.pow(c[2] - rgb1[2], 2)
    );
    var euclideanDistance2 = Math.sqrt(
      Math.pow(c[0] - rgb2[0], 2) +
        Math.pow(c[1] - rgb2[1], 2) +
        Math.pow(c[2] - rgb2[2], 2)
    );
...
// console.log(rybColorMixer.mix(['#ff0000', '#00ff00'], {result: 'rgb'}));

export default rybColorMixer;
